...
Usage (Automated Template Generation):
    python consolidates_v2.py --input-file pipeline_inputs_v2.yaml
...
def generate_dynamic_master_delivery_yml(inputs):
    """Builds a centralized entry router linking custom branches directly to their dedicated layout files."""
...
jobs:"""
    ...
    for b in inputs['branch_sequence']:
        ...
        if b_inputs.get('has_email_gate', False):
            workflow += f"""
...
def generate_dynamic_branch_pipeline_yml(b_name, b_inputs, inputs):
    """Creates a dedicated standalone environment workflow with custom project paths."""
    ...
    steps:
      ...
      - name: Run DCM Plan Against Target
        run: |
          ...
          snow dbt deploy <DBT_PROJECT_DEV> --source <DBT_SOURCE_DIR> --force -x
    ...
    steps:
      ...
      - name: Live Deploy to Database Target
        run: |
          ...
          snow dbt deploy <DBT_PROJECT_PROD> --source <DBT_SOURCE_DIR> --force -x
...
"""
    return (template
            ...
            .replace("<DCM_PROJECT_DIR>", inputs.get('dcm_project_dir', 'dcm_automation'))
            .replace("<DBT_SOURCE_DIR>", inputs.get('dbt_source_dir', './dcm_dbt_cicd'))
            .replace("<DBT_PROJECT_DEV>", inputs.get('dbt_project_dev', 'dbt_project_dev'))
            .replace("<DBT_PROJECT_PROD>", inputs.get('dbt_project_prod', 'dbt_project_prod')))


def generate_dynamic_business_gate_yml(b_name, b_inputs, inputs):
    """Compiles an asynchronous mailbox-polling clearance loop for specific release milestones."""
...
"""
    return (template
            ...
            .replace("<IMAP_SERVER>", inputs.get('imap_server', 'imap.gmail.com')))


def generate_dynamic_setup_runner_yml(inputs):
    """Generates the manual project initialization workflow tracking interactive runtime variables."""
    sf_branches = [b for b in inputs['branch_sequence'] if inputs['branch_data'][b].get('has_snowflake', False)]
    options_str = "\n".join([f"          - {b}" for b in sf_branches])
    default_env = sf_branches[0] if sf_branches else "qa"
    
    sample_branch = sf_branches[0] if sf_branches else inputs['branch_sequence'][-1]
    sample_db = inputs['branch_data'][sample_branch].get('sf_db', 'CICD_AUTOMATION_QA')
    sample_schema = inputs['branch_data'][sample_branch].get('sf_schema', 'UTILITIES')

    template = """name: "Manual One-Time Project Initialization"
on:
  workflow_dispatch:
    inputs:
      environment_tier:
...
"""
    return (template
            .replace("<OPTIONS_STR>", options_str)
            .replace("<DEFAULT_ENV>", default_env)
            .replace("<SF_ACCOUNT>", inputs['sf_account'])
            .replace("<SF_ROLE>", inputs['sf_role'])
            .replace("<SF_WAREHOUSE>", inputs['sf_warehouse'])
            .replace("<SAMPLE_DB>", sample_db)
            .replace("<SAMPLE_SCHEMA>", sample_schema))
        ...
        if e.status == 404 or "Branch not found" in str(e):
            ...
            try:
                repo.create_file(
                    ...
                    content=f"# {repo.name}\nScaffolded Dynamic Template Architecture.",
...
def lock_branch_via_api(repo, branch_name, token):
    ...
    if resp.status_code == 200:
        print(f"  [LOCKED] Target branch '{branch_name}' has been locked successfully.")
...
def apply_branch_protection(repo, branch_name, branch_config, token):
    ...
    try:
        ...
        if protection.get("require_pr", False) or is_locked:
            ...
            print(f"  [RULESET] Governance protection rules applied to branch '{branch_name}'.")
            if is_locked:
            ...
            print(f"  [RULESET] Loose baseline protection applied to open branch '{branch_name}'.")
    except GithubException as e:
        print(f"  [SKIPPED] Protection on '{branch_name}' skipped (requires premium tier or public visibility): {e}")
...
def main():
    ...
    print("\n[1/7] Running Cold-Start Empty Repo Checks...")
    initialize_empty_repo(repo, default_branch="main")

    # Step 2: Adaptive Topology Sequencing
    print("\n[2/7] Generating Specified Branch Topology Sequence...")
    for b_name in inputs['branch_sequence']:
        if b_name != "main":
            ensure_branch_exists(repo, b_name, source_branch="main")

    # Step 3: Branch Pointer Matching
    print(f"\n[3/7] Syncing Default Repository Pointer to '{inputs['default_branch']}'...")
    if repo.default_branch != inputs['default_branch']:
        repo.edit(default_branch=inputs['default_branch'])

    # Step 4: Temporarily Lift Branch Protections (CRITICAL FIX FOR IDEMPOTENCY)
    print("\n[4/7] Temporarily lifting protections to allow workflow adjustments...")
    for b_name in inputs['branch_sequence']:
        try:
            branch = repo.get_branch(b_name)
            branch.remove_protection()
            print(f"  [UNLOCKED] Protection cleared on '{b_name}' for file injection sync.")
        except GithubException:
            pass

    # Step 5: Environment Context Provisioning (With Approver Tracking)
    print("\n[5/7] Provisioning Environment Deployment Gates & Reviewers...")
    for b_name in inputs['branch_sequence']:
        b_inputs = inputs['branch_data'][b_name]
        if b_inputs['has_snowflake'] and b_inputs.get('approvers'):
            create_environment(repo, b_name, b_inputs['approvers'], token)

    # Step 6: File Injection Loop (Runs cleanly now that branches are unlocked)
    print("\n[6/7] Generating and Injecting Decentralized Modular Workflow Templates...")
    ...
    # 1. Inject environment-specific pipelines ONLY into their respective target branch contexts
    for b_name in inputs['branch_sequence']:
        b_inputs = inputs['branch_data'][b_name]
        
        if b_inputs['has_snowflake']:
            file_name = f".github/workflows/snowflake-pipeline-{b_name}.yml"
            commit_file_to_github(repo, file_name, generate_dynamic_branch_pipeline_yml(b_name, b_inputs, inputs), branch=b_name)
            
            if b_inputs.get('has_email_gate', False):
                gate_file_name = f".github/workflows/business-email-gate-{b_name}.yml"
                commit_file_to_github(repo, gate_file_name, generate_dynamic_business_gate_yml(b_name, b_inputs, inputs), branch=b_name)
    
    # 2. Inject global router, pr gate, AND the newly restored manual project setup workflow into ALL branches
    pr_gate_content = generate_dynamic_pr_gate_yml(inputs)
    master_delivery_content = generate_dynamic_master_delivery_yml(inputs)
    setup_runner_content = generate_dynamic_setup_runner_yml(inputs)
    
    for b_name in inputs['branch_sequence']:
        commit_file_to_github(repo, ".github/workflows/pr-gate.yml", pr_gate_content, branch=b_name)
        commit_file_to_github(repo, ".github/workflows/snowflake-delivery-master.yml", master_delivery_content, branch=b_name)
        commit_file_to_github(repo, ".github/workflows/manual-project-initialization.yml", setup_runner_content, branch=b_name)

    # Step 7: Clamp Down Branch Governance & Locks (Executed LAST)
    print("\n[7/7] Binding Branch Governance Rulesets & Trunk Commit Locks...")
    for b_name, b_cfg in derived_yaml_config["branches"].items():
        apply_branch_protection(repo, b_name, b_cfg, token)

    print(f"\n{'='*60}")
    print(" GATED PIPELINE MATRIX GENERATED AND DEPLOYED SUCCESSFULLY!")
    print("="*60 + "\n")
